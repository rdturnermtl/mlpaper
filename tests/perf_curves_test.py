...
def test_nv_binary_clf_curve():
    ...
    prec2, rec2, thresholds2 = precision_recall_curve(y_bool, y_pred, pos_label=True, sample_weight=sample_weight)
...
def test_binary_clf_curve():
    ...
    if np.random.rand() <= 0.2:
        sample_weight = np.maximum(np.random.multinomial(N, p, size=n_boot), 1e-6)

    fps, tps, thresholds = pc._binary_clf_curve(y_bool, y_pred, sample_weight)
    (fpr, tpr, kind), thresholds_roc = pc.roc_curve(y_bool, y_pred, sample_weight)
    assert kind == "linear"
    (rec, prec, kind), thresholds_pr = pc.recall_precision_curve(y_bool, y_pred, sample_weight)
    assert kind == "previous"
    (rec_gain, prec_gain, kind), thresholds_prg = pc.prg_curve(y_bool, y_pred, sample_weight)
    assert kind == "previous"
    ...
    if sample_weight is None:
        fps2, tps2, thresholds2 = _nv_binary_clf_curve(y_bool, y_pred, np.ones(N, dtype=int))
        ...
        rec2, prec2, thresholds_pr2 = _nv_recall_precision_curve(y_bool, y_pred)
    ...
    for ii in range(n_boot):
        ...
        fps2, tps2, thresholds2 = _nv_binary_clf_curve(y_bool, y_pred, weight_curr)
        assert np.allclose(fps2, fps[ii, :])
        assert np.allclose(tps2, tps[ii, :])
        assert np.allclose(thresholds2, thresholds)

        fpr2, tpr2, thresholds_roc2 = _nv_roc_curve(y_bool, y_pred, weight_curr)
        assert np.allclose(fpr2, fpr[ii, :])
        assert np.allclose(tpr2, tpr[ii, :])
        assert np.allclose(thresholds_roc2, thresholds)

        rec2, prec2, thresholds_pr2 = _nv_recall_precision_curve(y_bool, y_pred, weight_curr)
        assert np.allclose(rec2, rec[ii, :])
        assert np.allclose(prec2, prec[ii, :])
        assert np.allclose(thresholds_pr2, thresholds_pr)

        rec_gain2, prec_gain2, thresholds_prg2 = _nv_prg_curve(y_bool, y_pred, weight_curr)
