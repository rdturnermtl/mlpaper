...
def decimalize(perf_tbl, err_digits=2, pval_digits=4, default_digits=5, EB_limit={}):
    ...
    perf_tbl_dec = pd.DataFrame(index=perf_tbl.index, columns=perf_tbl.columns, dtype=object)
    ...
    for metric in metrics:
        ...
        for method in methods:
            EB = create_decimal(perf_tbl.loc[method, (metric, ERR_COL)], err_digits, decimal.ROUND_CEILING)
            ...
            if EB.is_finite() and (not EB.is_zero()):
                ...
                mu = create_decimal(perf_tbl.loc[method, (metric, MEAN_COL)], default_digits, decimal.ROUND_HALF_UP)
            ...
            pval = pval.quantize(decimal_1ek(-pval_digits), rounding=decimal.ROUND_CEILING)
...
def print_estimate(
    mu, EB, shift=0, min_clip=D_NINF, max_clip=D_INF, below_fmt=BELOW_FMT, above_fmt=ABOVE_FMT, non_finite_fmt={}
):
    r"""Convert a mean and error bar pair in `Decimal` to a string.
...
def find_shift(mean_list, err_list, shift_mod=1):
    ...
    for shift in L:
        ...
        max_len = max(str_print_len(print_estimate(mu, EB, shift)) for mu, EB in zip_list)
...
def pad_num_str(num_str_list, pad=" "):
    ...
    L = [ss + pad * (max_right - (len(ss) - find_last_dig(ss))) for ss in num_str_list]
    return L


def format_table(perf_tbl_dec, shift_mod=None, pad=True, crap_limit_max={}, crap_limit_min={}, non_finite_fmt={}):
    r"""Format a performance table that is already in decimal form to one that
    ...
    cols = pd.MultiIndex.from_product([metrics, FMT_STATS], names=[METRIC, STAT])
    ...
    for metric in metrics:
        ...
        assert all_same([p.as_tuple().exponent for p in perf_tbl_dec[(metric, PVAL_COL)] if p.is_finite()])
        ...
        if shift_mod is None:  # => no shifting at all
            ...
            best_shift = find_shift(mean_series[idx].tolist(), err_series[idx].tolist(), shift_mod=shift_mod)
        shifts[metric] = best_shift

        # Now actually do it
        for method in methods:
            mu, EB = mean_series[method], err_series[method]
            estimate_str = print_estimate(mu, EB, best_shift, min_clip, max_clip, non_finite_fmt=non_finite_fmt)
        ...
        if pad:  # Overwrite with padded values if requested
            perf_tbl_str.loc[:, (metric, EST_COL)] = pad_num_str(perf_tbl_str[(metric, EST_COL)])
...
def table_to_latex(perf_tbl_str, shifts, unit_dict, use_prefix=True):
    ...
    new_headers = adjust_headers(perf_tbl_str.columns, shifts, unit_dict, use_prefix=use_prefix, use_tex=True)
    ...
    perf_tbl_str = pd.DataFrame(
        data=perf_tbl_str.values, columns=new_headers, index=perf_tbl_str.index.values, dtype=str
    )
    latex_str = perf_tbl_str.to_latex(escape=False, column_format=align, index_names=False)
    return latex_str
...
def table_to_string(perf_tbl_str, shifts, unit_dict, use_prefix=True):
    ...
    new_headers = adjust_headers(perf_tbl_str.columns, shifts, unit_dict, use_prefix=use_prefix, use_tex=False)
...
def just_format_it(
    ...
    perf_tbl_str, shifts = format_table(
        perf_tbl_dec,
        shift_mod=shift_mod,
